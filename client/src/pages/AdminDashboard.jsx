...
import {
  ...
  FaPlusCircle,
  FaMoneyBillWave,
} from "react-icons/fa"; // Icons for modules
...
const cardVariants = {
  ...
  hover: {
    scale: 1.05,
    boxShadow:
      "0 15px 30px rgba(0, 0, 0, 0.5), 0 0 10px rgba(78, 117, 255, 0.4)", // Stronger shadow/glow
...
const dashboardModules = [
  ...
  },
  {
    ...
    icon: FaCheckSquare,
    description:
      "Review influencer applications for campaigns and manage approvals.",
  ...
  },
  {
    ...
    description:
      "Review submitted campaign deliverables, add comments, and approve payouts.",
  ...
  },
  {
    ...
    icon: FaUsers,
    description:
      "Manage administrators, brands, and the full influencer database.",
          ...
          <div className="flex flex-col gap-2">
            {auth?.user?.role === "superadmin" && (
              <Button
                ...
                variant="ghost"
                size="sm"
                className="text-purple-300 hover:bg-gray-700/50"
              >
...
export default function AdminDashboard() {
  ...
  const canCampaignCreate =
    auth?.user?.role === "superadmin" ||
    auth?.user?.permissions?.includes("campaign:create");
  const canPaymentsApprove =
    auth?.user?.role === "superadmin" ||
    auth?.user?.permissions?.includes("payouts:approve");
